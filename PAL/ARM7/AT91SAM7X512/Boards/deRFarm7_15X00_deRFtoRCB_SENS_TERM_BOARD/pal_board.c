...
void pal_led_init(void)
{
    ...
    PIN_SET_AS_PIO_OUTPUT(LED_ADDR_0, PIO_B);
    PIN_SET_AS_PIO_OUTPUT(LED_ADDR_1, PIO_A);

    /* switch LEDs off - both are low active. */
    PIN_SET(LED_ADDR_0, PIO_B);
    PIN_SET(LED_ADDR_1, PIO_A);
...
{
    /* change status of the respective LED */
    switch (led_setting)
    {
        case LED_ON:
            led_state |= (1<<led_no);
    ...
    /*
     * Both LEDs need to be updated, since several peripherals are dealing
     * with the same port for this board (USB, LEDs, Button).
     */
    PIN_SET_AS_PIO_OUTPUT(LED_ADDR_0, PIO_B);
    PIN_SET_AS_PIO_OUTPUT(LED_ADDR_1, PIO_A);

    if (led_state&(1<<LED_0))     { PIN_CLEAR(LED_ADDR_0, PIO_B); }
    else                         { PIN_SET(LED_ADDR_0, PIO_B);    }

    if (led_state&(1<<LED_1))     { PIN_CLEAR(LED_ADDR_1, PIO_A); }
    else                         { PIN_SET(LED_ADDR_1, PIO_A);     }
...
static void led_helper_func(void)
{
    ...
    PIN_SET(DMUX_PIN_0, PIO_A);
    PIN_CLEAR(DMUX_PIN_1, PIO_B);

    /* Enable desired LED state - toggle WR# */
...
{
    /* nothing to do for Button on STB */
}
...
button_state_t pal_button_read(button_id_t button_no)
{
    ...
    /* Switch port to input. Read pin state */
    PIN_SET_AS_PIO_INPUT(BUTTON_ADDR_1, PIO_B);
    PIN_GET_STATUS(cur_button_state, BUTTON_ADDR_1, PIO_B);

    /* toggle RD# back */
    PIN_SET(RD_PIN, PIO_A);

    /* Switch port back to output. */
    PIN_SET_AS_PIO_OUTPUT(BUTTON_ADDR_1, PIO_B);
...
#endif /* deRFarm7_15X00_deRFtoRCB_SENS_TERM_BOARD */
