...
#if (BOARD_TYPE == deRFarm7_15X00_deRFtoRCB_BREAKOUT_BOARD)
...
button_state_t pal_button_read(button_id_t button_no)
{
    ...
    // --- button on RF2RCB-Adapter ---
    PIN_GET_STATUS(cur_button_state, button_pin, PIO_A);

    // button pins are low-active !!
    return (cur_button_state?BUTTON_OFF:BUTTON_PRESSED);
}
...
#endif /* deRFarm7_15X00_deRFtoRCB_BREAKOUT_BOARD */
