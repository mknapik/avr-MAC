 ...
 * @file pal_timer.c
 *
 * @brief Timer related functions for AVR 8-Bit MCUs
 *
 * This file implements timer related functions for AVR 8-Bit MCUs.
 *
 * $Id: pal_timer.c 20047 2010-01-28 11:59:48Z sschneid $
 *
...
void  pal_timer_delay(uint16_t delay)
{
    ...
    {
        uint16_t target_time = TCNT1 + delay;

        if (target_time > TCNT1)
        {
            /* Target time is within current span of 16 bit count (0xFFFF) */
            while (target_time > TCNT1)
            {
...
static inline uint32_t gettime(void)
{
    ...
    do
    {
        ...
        current_time = current_time | TCNT1;
        /*
...
#if ((TOTAL_NUMBER_OF_TIMERS > 0) || defined(DOXYGEN))
/**
 * @brief Programs the output compare register OCR1A
 *
 * This function programs the OCR1A register with the timeout value of the
...
#if defined(DOXYGEN)
/**
 * @brief ISR for high priority timer, triggered by OCR1B
 *
